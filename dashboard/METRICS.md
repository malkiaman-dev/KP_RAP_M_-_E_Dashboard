...
`Error_log/Daily_Error_Log.xlsx` (generated by `DQA_Script/run_dqa.py`) for
**Tracking** (baseline + new sample), **Household**, and **Girls** only.
Issues are split into **Critical** (must-fix) and **Quality** (flag for review).
Listing / School / Driver and cross-survey integrity checks (e.g. *Tracking vs
Household*) are excluded. Logic lives in `src/lib/data/error-metrics.ts`.
