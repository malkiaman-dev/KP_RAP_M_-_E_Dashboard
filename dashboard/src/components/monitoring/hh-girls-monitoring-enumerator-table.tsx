...
import {
  buildHhGirlsEnumeratorReportFilename,
  downloadHhGirlsEnumeratorReport,
} from "@/lib/export/hh-girls-monitoring-excel";
...
type EnumeratorRow = HhGirlsEnumeratorPerformance & {
  hhLeftFromTarget: number;
};

const columns: { key: SortKey; label: string; numeric?: boolean }[] = [
  { key: "name", label: "Enumerator" },
  { key: "expectedCompleted", label: "Target HH Count", numeric: true },
  { key: "submissions", label: "Forms", numeric: true },
  { key: "completedHouseholds", label: "Completed HH", numeric: true },
  { key: "hhLeftFromTarget", label: "HH Left from Target", numeric: true },
  { key: "successRate", label: "Success %", numeric: true },
  { key: "activeDays", label: "Days", numeric: true },
  { key: "avgCompletedPerDay", label: "Avg/Day (HH)", numeric: true },
  { key: "targetAttainment", label: "Target % (HH)", numeric: true },
  { key: "avgSubmissionsPerDay", label: "Avg/Day (Forms)", numeric: true },
  {
    key: "submissionTargetAttainment",
    label: "Target % (Forms)",
    numeric: true,
  },
];

export function HhGirlsMonitoringEnumeratorTable({
  metrics,
  loading,
  filters,
  districtOptions,
  ...
  const [sortKey, setSortKey] = useState<SortKey>("completedHouseholds");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const scrollRef = useRef<HTMLDivElement>(null);
  const [statusMetric, setStatusMetric] = useState<"hh" | "forms">("hh");
  ...
  const handleScroll = () => {
    ...
    const next =
      maxScroll > 0 && el.scrollLeft >= maxScroll / 2 ? "forms" : "hh";
  ...
  const rows = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      const av =
        sortKey === "hhLeftFromTarget"
          ? a.expectedCompleted - a.completedHouseholds
          : a[sortKey];
      const bv =
        sortKey === "hhLeftFromTarget"
          ? b.expectedCompleted - b.completedHouseholds
          : b[sortKey];
      ...
      <div className="flex flex-col gap-3 border-b border-border/60 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground">
            {metrics.enumeratorsOnTrack} of {metrics.activeEnumerators}{" "}
            enumerators meeting the {metrics.dailyHhTarget} HH/day target on
            average · forms target {metrics.dailyFormsTarget}/day
          </p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div
            className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/30 px-3 py-1.5"
            title={`${metrics.totalSubmissions.toLocaleString()} forms of ${metrics.expectedSubmissions.toLocaleString()} expected (${metrics.dailyFormsTarget}/day)`}
          >
          ...
          <button
            ...
            onClick={() => {
              const filename = filters
                ...
                : `All_Districts_HH_Enumerator_Performance_${new Date().toISOString().slice(0, 10)}.xlsx`;
              downloadHhGirlsEnumeratorReport(rows, filename);
            }}
              ...
              >
                ...
                <td
                  className={cn(
                    ...
                    e.avgCompletedPerDay >= e.dailyHhTarget
                      ? "text-teal"
                      : "text-red-600"
                  )}
                >
                  {Math.round(e.avgCompletedPerDay)}
                </td>
                ...
                <td
                  className={cn(
                    "px-4 py-2.5 text-right tabular-nums font-medium",
                    e.avgSubmissionsPerDay >= e.dailyFormsTarget
                      ? "text-teal"
                      : "text-red-600"
                  )}
                >
