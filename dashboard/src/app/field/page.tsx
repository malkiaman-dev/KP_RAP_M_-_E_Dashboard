...
export default function FieldErrorsPage() {
  ...
  const scopedBase = data?.allErrors ?? [];
  ...
  const panelFilterOptions = useMemo(() => {
    ...
    return {
      ...
      severities: scopedMetrics.filterOptions.severities,
      dateRange:
        data?.filterOptions?.dateRange ?? scopedMetrics.filterOptions.dateRange,
    };
  }, [scopedBase, filters, data?.filterOptions]);
  ...
  return (
    <div>
      <PageHero
        ...
        accent="Field"
        description="Review critical and quality issues for your district. Filter by error title, enumerator, and more."
        loading={isLoading}
